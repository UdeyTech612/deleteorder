...
namespace Udeytech\DeleteOrder\Plugin\Invoice;
...
/**
 * Class PluginAfter
 * @package Udeytech\DeleteOrder\Plugin\Invoice
 */
    ...
    public function afterGetBackUrl(View $subject, $result)
    {
        if ($this->isAllowedResources()) {
            ...
            if ($subject->getRequest()->getFullActionName() == 'sales_order_invoice_view') {
                $subject->addButton(
                    'Udeytech-delete',
                    ['label' => __('Delete'), 'onclick' => 'confirmSetLocation(\'' . $message . '\',\'' . $this->getDeleteUrl($params['invoice_id']) . '\')', 'class' => 'Udeytech-delete'],
                    -1
                );
            }
        }
